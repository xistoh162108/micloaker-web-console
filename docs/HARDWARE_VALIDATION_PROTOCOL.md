...
Each run detail page also provides **DAQ Evidence Draft**, a downloadable text note that pre-fills the DAQ validation checklist from the saved run metadata, metrics, raw `.bin` path, run log state, and plot artifact status. Review physical wiring and measured values before copying the draft into `/ops`.
