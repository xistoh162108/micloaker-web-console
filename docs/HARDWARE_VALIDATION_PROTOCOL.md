...
Each run detail page also provides **DAQ Evidence Draft**, **Mac Evidence Draft**, and **Play+Record Draft** downloads. These text notes pre-fill the relevant validation checklists from saved run metadata, Helper status, metrics, raw `.bin` path, run log state, WAV outputs, and plot artifact status. The Compare page provides an attenuation-pair **Evidence Draft** from saved comparison JSON/CSV and plot status. Review physical wiring, routing, and measured values before copying any draft into `/ops`.
Each validation record also stores **Evidence completeness**: the console records which checklist labels have non-empty `label: value` evidence lines and which are still missing in JSONL, Markdown reports, readiness reports, and exported evidence packages. A `pass` record still reflects operator judgment, but incomplete checklist evidence remains visible for review.
...
Terminal-only runs can also generate artifact-based drafts with `scripts/lab_readiness_check.py --write-evidence-draft <daq|mac|play_and_record|attenuation> --draft-session-id <session_id> --draft-run-id <run_id> --evidence-draft-file evidence.txt`; use `--draft-compare-id <compare_id>` for attenuation drafts.
...
Run `scripts/lab_readiness_check.py --write-evidence-template <gate> --evidence-template-file evidence.txt` to create a fillable evidence note with the gate checklist, evidence hint, and record command.
