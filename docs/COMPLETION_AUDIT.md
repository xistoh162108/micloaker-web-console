...
| Hardware validation records with workflow navigation | Proven for text persistence | `/ops` and `scripts/lab_readiness_check.py --record-gate ...` record operator-entered validation evidence, `--validation-plan` and `/ops/validation/plan` provide ordered lab gate instructions, `--record-evidence-file` supports longer terminal evidence notes, gate-specific evidence hints are exposed, the Use checklist draft helper fills fields, Next action links route to DAQ run creation/Mac Helper/Compare/file review, and exports include JSONL/Markdown evidence. |
