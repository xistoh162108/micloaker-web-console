...
| Hardware validation records with workflow navigation | Proven for text persistence | `/ops` records operator-entered validation evidence, exposes gate-specific evidence hints, provides a Use checklist draft helper, provides Next action links to DAQ run creation, Mac Helper, Compare, and file review, and exports JSONL/Markdown evidence. |
