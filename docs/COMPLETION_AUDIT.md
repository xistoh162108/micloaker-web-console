...
| Hardware validation records with workflow navigation | Proven for text persistence | `/ops` and `scripts/lab_readiness_check.py --record-gate ...` record operator-entered validation evidence, expose gate-specific evidence hints, provide a Use checklist draft helper, provide Next action links to DAQ run creation, Mac Helper, Compare, and file review, and export JSONL/Markdown evidence. |
