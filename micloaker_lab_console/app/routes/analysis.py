...
from ..services.recorder import finalize_run, mark_finalization_failed
...
def finalize(request: Request, session_id: str, run_id: str, overwrite_existing: bool = Form(False)):
    ...
    try:
        run_job(
            ...
            "manual_finalize",
            log_path,
            lambda: finalize_run(workspace, run, trigger="manual", overwrite_derived=overwrite_existing),
        )
    except FileNotFoundError as exc:
        failure = FileNotFoundError(f"raw .bin does not exist: {exc}")
        mark_finalization_failed(
            workspace,
            run,
            trigger="manual",
            exc=failure,
            failure_label="Manual finalization failed because the saved raw .bin is missing; record or import raw data before retrying.",
        )
        raise HTTPException(status_code=400, detail=_raw_bin_missing_error(run_id, str(exc))) from exc
    except FileExistsError as exc:
        raise HTTPException(status_code=409, detail={"error_code": "DERIVED_OUTPUT_EXISTS", "message": str(exc), "suggestion": "Enable overwrite existing derived outputs, or keep current WAV/plot outputs."}) from exc
        ...
        mark_finalization_failed(
            ...
            trigger="manual",
            exc=exc,
            failure_label="Manual finalization failed because the saved raw .bin is invalid; inspect the run log and replace or re-record the raw data.",
        )
        ...
        mark_finalization_failed(
            ...
            trigger="manual",
            exc=exc,
            failure_label="Manual finalization failed because the run configuration is invalid; inspect the run log and fix the metadata before retrying.",
        )
...
def _raw_bin_missing_error(run_id: str, message: str) -> dict[str, str]:
    return {
        ...
        "suggestion": "Record the run or import a raw float64 .bin before finalizing report-grade metrics.",
    }
...
def _invalid_raw_bin_error(run_id: str, message: str) -> dict[str, str]:
    return {
        ...
        "suggestion": "Replace the saved raw file with a non-empty little-endian float64 voltage .bin before finalizing report-grade metrics.",
    }
    ...
    if not (base / "session.json").is_file():
        raise HTTPException(
            ...
            detail={
                ...
                "suggestion": "Open the Sessions page and choose an existing session before finalizing a run.",
            },
        )
    if not (base / "metadata" / f"{safe_name(run_id)}.json").is_file():
        raise HTTPException(
            ...
            detail={
                ...
                "suggestion": "Open the session page and choose an existing run before finalizing.",
            },
        )
