...
The UI standard is DaisyUI component vocabulary implemented locally in vanilla CSS, so the app keeps the no-build-step FastAPI/Jinja2 deployment model while using consistent `btn`, `card`, `stats`, `badge`, form, and table patterns. The visual style is a scientific instrument console: neutral lab surfaces, low-contrast chart chrome, restrained semantic status colors, and cyan/blue data emphasis. Live charts support hover readouts and crosshair inspection for faster waveform/PSD/spectrogram review. Core experiment operation is not hidden behind dashboard tabs.
...
Use `/exports/ops-validation.zip` or the **Ops Evidence ZIP** button on `/ops` to download the same workspace-level validation/readiness evidence without bundling a session.
...
Run detail pages provide DAQ, Mac playback, and play-and-record evidence draft downloads from saved run metadata and artifacts. Compare pages provide attenuation-pair evidence draft downloads from saved comparison files and plot status. Review physical setup before copying any draft into `/ops`.
Terminal-only operators can generate the same artifact-based drafts with `scripts/lab_readiness_check.py --write-evidence-draft <daq|mac|play_and_record|attenuation> --draft-session-id <session_id> --draft-run-id <run_id> --evidence-draft-file evidence.txt` or `--draft-compare-id <compare_id>` for attenuation drafts.
